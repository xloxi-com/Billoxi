...
const HEADING = "Download invoice";
const CONVERT_LABEL = "Convert to invoice";
...
function Extension() {
  ...
  const handleConvert = async () => {
    ...
    setError("");
    try {
      // Relative HTTPS-proxied path — never absolute http://localhost
  ...
  return (
    <s-admin-action heading={HEADING} loading={busy && !error}>
      ...
      {error ? (
        ...
        <s-text>Checking invoice…</s-text>
      ) : needsConvert ? (
        <s-banner heading="Not an invoice yet" tone="warning">
          Convert this order to an invoice first, then download the PDF (same as
          in Billoxi).
        </s-banner>
      ) : saved ? (
