...
const HEADING_DOWNLOAD = "Return download";
const HEADING_CONVERT = "Convert to return & download";
  ...
  return (
    <s-admin-action heading={heading} loading={checking && !error}>
      ...
      {error ? (
        ...
        <s-text>Checking return status…</s-text>
      ) : saved ? (
        <s-banner
        ...
        >
          {didConvert
            ? "Return created in Billoxi. PDF downloading — check downloads."
            : "PDF downloading — check your downloads folder."}
        </s-banner>
      ) : showConvert ? (
        <s-banner heading="Convert to return & download" tone="warning">
        ...
        <s-banner heading="Return download" tone="info">
          This order already has a return document. Tap to download the PDF.
        </s-banner>
      )}
    </s-admin-action>
  );
