...
const DOCUMENT_KIND = "sales-order";
const HEADING_DOWNLOAD = "Sales order download";
...
function Extension() {
  ...
  return (
    <s-admin-action heading={HEADING_DOWNLOAD} loading={checking && !error}>
      <s-button
        slot="primary-action"
        variant="primary"
        onClick={handlePrimary}
        disabled={working || checking || !readyUrl}
      >
        {working ? "Opening…" : HEADING_DOWNLOAD}
      </s-button>
      ...
      {error ? (
        ...
        <s-text>Preparing sales order PDF…</s-text>
      ) : saved ? (
        ...
        <s-banner heading="Sales order download" tone="info">
          Tap to download the sales order PDF.
        </s-banner>
      )}
    </s-admin-action>
  );
